...
interface GoldExchangePrintProps {
  exchange: GoldExchange
}

export function GoldExchangePrint({ exchange }: GoldExchangePrintProps) {
  ...
  const finalHsnCode = exchange.hsn_code || hsnCode || '7113'
  const customerName = exchange.bills?.customers?.name || 'N/A'
  const billNo = exchange.bills?.bill_no || `EX${exchange.id}`
  const exchangeDate = exchange.bills?.bill_date || exchange.created_at
        ...
        <div className="purchase-a4">
          <div className="purchase-bill-box">
            ...
            <div className="purchase-top-header">
              <div>
                Mr./Mrs: <span className="purchase-pink-write">{customerName}</span>
              </div>
              <div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-start', gap: '12px', marginTop: '4mm' }}>
              <img src="/Logo.png" alt="AM JEWELLERY Logo" style={{ height: '50px', width: 'auto', objectFit: 'contain' }} />
              <div className="purchase-title" style={{ marginTop: 0 }}>AM JEWELLERY</div>
            </div>

            <div className="purchase-sub-info">
              <div>Phone: 9845105226</div>
              <div>Mob: 9986865090</div>
              <div>Mob: 9620921903</div>
            </div>

            <div className="purchase-sub-info">
              <div>Address: No. 4, "B" Street, Lal Masjid Road Cross, Shivajinagar, Bangalore – 560 051</div>
              <div>
                Date: <span className="purchase-pink-write">{formatDate(exchangeDate)}</span>
              </div>
            </div>
              ...
              <tbody>
                <tr>
                  ...
                  <td className="purchase-pink-write">
                    {formatWeight(exchange.weight)}
                  </td>
                  <td className="purchase-pink-write">{exchange.purity || '-'}</td>
                  <td className="purchase-pink-write">
                    {formatRate(exchange.rate_per_gram)}
                  </td>
                  <td className="purchase-pink-write">{formatAmount(exchange.total_value)}</td>
                </tr>
              </tbody>
            </table>
            ...
            <div className="purchase-footer-row">
              ...
              <div>SGST 1.5% —</div>
              <div>
                Total: <span className="purchase-pink-write">{formatAmount(exchange.total_value)}</span>
              </div>
            </div>

            <div className="purchase-signature">For AM JEWELLERY</div>
          </div>
        </div>
      </div>
    </>
